...
const ImageThumbnail = ({file}:{file:Models.Document}) => (
    <div className='file-details-thumbnail'>
        ...
        <div className='flex flex-col'>
            ...
            <FromattedDateTime date={file.$createdAt} className='caption' />
        </div>
    </div>
)

const DetailRow = ({lable,value}:{lable:string; value:string;}) => (
    <div className='flex'>
        <p className='file-details-label text-left'>{lable}</p>
  ...
  return (
    <>
        ...
        <div className='space-y-4 px-2 pt-2'>
            <DetailRow lable='Format:' value={file.extension} />
            <DetailRow lable='Size:' value={convertFileSize(file.size)} />
            <DetailRow lable='Owner:' value={file.owner.fullName} />
            <DetailRow lable='Last edit:' value={formatDateTime(file.$updatedAt)} />
        </div>
    </>
  )
}
